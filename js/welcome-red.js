...
const trackingEye = document.querySelector('.index-tracking-eye');
...
function getEyeGeometry() {
  ...
  return {
    ...
    centerY: rect.top + rect.height * 0.488,
...
function moveTrackingEye(event) {
  const { rect, centerX, centerY, activationRadius } = getEyeGeometry();
  const dx = event.clientX - centerX;
  const dy = event.clientY - centerY;
  const angle = Math.atan2(dy, dx);
  const maxX = rect.width * 0.014;
  const maxY = rect.height * 0.010;
  ...
  if (trackingEye) {
    trackingEye.style.transform = `translate(calc(-50% + ${x}px), calc(-50% + ${y}px))`;
  }
...
document.addEventListener('mousemove', moveTrackingEye, { passive: true });
